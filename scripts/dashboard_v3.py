...
Streamlit + Plotly â reads from Supabase (apgia schema)
...
st.set_page_config(
    ...
    page_title="APGIA â Cycling Analytics",
...
# ââ Supabase connection (cached) ââââââââââââââââââââââââââââââ
...
# ââ Auth: Azure AD via Supabase OAuth (PKCE, static verifier) ââââ

REDIRECT_URL = "https://apgia-dashboard.streamlit.app/"

# Static PKCE pair â single-user app, no interception risk

PKCE_VERIFIER = "apgia-anna-graboski-static-pkce-verifier-2026"

PKCE_CHALLENGE = "eOS0O-C-RkqlAjn5MD8sbKAzHivX7_A2tYP98toKXKs"
...
def auth_guard():
    ...
    PKCE OAuth with static verifier.

    - PKCE returns ?code= as query param (Streamlit reads it natively)

    - Static verifier avoids session_state loss between tabs/reloads

    - Anon key client for auth exchange

    """

    # Step 1: Check if we got ?code= back from Supabase
    ...
    # Step 2: Check session state (already logged in)
...
# ââ Theme colors ââââââââââââââââââââââââââââââââââââââââââââââââ
...
# ââ Data Loading (cached, from Supabase) ââââââââââââââââââââââ
...
st.sidebar.title("ð´ââï¸ APGIA")
...
page = st.sidebar.radio(
    ...
    [
        ...
        "ð PMC (Fitness/Fadiga)",

        "â¡ PotÃªncia & EficiÃªncia",
...
try:
    ...
    if cycling.empty:
        ...
        st.error("â ï¸ Sem dados em cycling_sessions.")
    ...
    st.sidebar.subheader("ð PerÃ­odo")
    ...
    st.sidebar.caption(f"{start_date} â {end_date}")



except Exception as e:

    st.error(f"â ï¸ Erro ao carregar dados: {e}")
...
if page == "ð VisÃ£o Geral":
    ...
    st.caption(f"ð {start_date} â {end_date} â¢ {len(bike)} treinos")
    ...
    col2.metric("DistÃ¢ncia", f"{bike['distance_km'].sum():,.0f} km")

    col3.metric("Horas", f"{bike['duration_min'].sum() / 60:,.0f}h")

    col4.metric("ElevaÃ§Ã£o", f"{bike['total_ascent'].sum():,.0f} m")
    ...
    fig.add_trace(
        ...
        go.Bar(x=monthly["timestamp"], y=monthly["km"], name="DistÃ¢ncia (km)",
    ...
    with col1:
        ...
        weekday_labels = ["Seg", "Ter", "Qua", "Qui", "Sex", "SÃ¡b", "Dom"]
    ...
    with col2:
        ...
        fig_dist.update_layout(**PLOT_LAYOUT, height=300, title="DistribuiÃ§Ã£o de DistÃ¢ncia (km)")
...
# âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
elif page == "ð PMC (Fitness/Fadiga)":

    st.title("Performance Management Chart")

    st.caption(f"ð {start_date} â {end_date}")
    ...
    if tsb_val > 15:
        ...
        estado = "ð¡ Equilibrada"

    elif tsb_val > -15:

        estado = "ð  Fadiga leve"

    else:

        estado = "ð´ Fadiga acumulada"
    ...
    fig.add_trace(go.Bar(x=pmc["date"], y=pmc["tss"], name="TSS diÃ¡rio",
    ...
    st.subheader("Ramp Rate (variaÃ§Ã£o semanal do CTL)")
...
# PAGE: POTÃNCIA & EFICIÃNCIA

# âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

elif page == "â¡ PotÃªncia & EficiÃªncia":

    st.title("PotÃªncia & EficiÃªncia")

    bike = filter_by_date(cycling)

    valid = bike[bike["avg_power"].notna() & bike["avg_heart_rate"].notna()].copy()

    st.caption(f"ð {start_date} â {end_date} â¢ {len(valid)} treinos com potÃªncia")
    ...
    if len(ef_data) > 0:
        ...
        fig_ef.add_trace(go.Scatter(x=ef_data["timestamp"], y=ef_data["ef_roll"],
                                     ...
                                     mode="lines", name="MÃ©dia mÃ³vel (20)",
    ...
    with col1:
        ...
        st.subheader("PotÃªncia por Trimestre")
    ...
    if len(dec_data) > 0:
        ...
        fig_dec.add_trace(go.Scatter(x=dec_data["timestamp"], y=dec_data["dec_roll"],
                                      ...
                                      mode="lines", name="MÃ©dia mÃ³vel (15)",
...
# PAGE: RECOVERY & SONO (NEW â Whoop data)

# âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ

elif page == "ð Recovery & Sono":
    ...
    if whoop.empty:
        ...
        st.warning("â ï¸ Sem dados Whoop.")
    ...
    st.caption(f"ð {start_date} â {end_date} â¢ {len(w)} dias")
    ...
    st.subheader("EstÃ¡gios de Sono")
...
# âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
    ...
    if len(recent_w) > 0:
        ...
        col2.metric("MÃ©dia 30 dias", f"{recent_w['weight_kg'].mean():.1f} kg")

        col3.metric("VariaÃ§Ã£o",
    ...
    fig_w.add_trace(go.Scatter(x=w_sorted["date"], y=w_sorted["w_roll"], mode="lines",
                                ...
                                name="MÃ©dia 30 dias", line=dict(color=COLORS["ef"], width=3)))
...
st.sidebar.caption("APGIA v3 â¢ Supabase + Azure AD SSO")

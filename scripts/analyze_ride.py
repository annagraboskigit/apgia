"""
analyze_ride.py — Análise pós-treino com contexto completo (Supabase)
Uso:
  python analyze_ride.py --manual
  python analyze_ride.py --fit arquivo.fit
  python analyze_ride.py --quick 90 180 150 45   (duração_min avgW NP TSS)

Lê de: Supabase (apgia schema) via pipeline.query
Gera:  output/last_ride_analysis.json
"""
...
from pipeline.query import get_pmc, get_cycling, get_whoop, get_lab_results, get_weight

OUTPUT = Path("output")
OUTPUT.mkdir(exist_ok=True)

FTP = 219  # Update or read from config
...
def input_quick(args):
    ...
    return {
        ...
        'avg_hr': 0, 'max_hr': 0, 'avg_cadence': 0,
        'rpe': 5, 'feel': 'ok', 'notes': '',
        'pre_recovery': None, 'pre_hrv': None,
    }
...
def calc_derived(ride):
    ...
    # W/kg — pega peso mais recente do Supabase
    weight_df = get_weight()
    if not weight_df.empty:
        weight = weight_df.iloc[-1].get('weight_kg', 68)
    else:
        weight = 68
    r['weight_used'] = weight
...
# ═══════════════════════════════════════════════════════════════
# ANALYSIS — reads from Supabase
# ═══════════════════════════════════════════════════════════════

def get_current_state(ride_date):
    """Pega estado atual (PMC + Whoop) do Supabase"""
    state = {}
    ...
    whoop = get_whoop()
    if not whoop.empty:
        recent = whoop[whoop['date'] <= rd].tail(1)
        if len(recent) > 0:
            r = recent.iloc[0]
            for col in ['recovery_score', 'hrv_rmssd', 'resting_hr', 'skin_temp',
                        'sleep_duration_min', 'deep_sleep_min', 'day_strain']:
                if col in r.index and pd.notna(r[col]):
                    state[col] = r[col]

        # 7-day averages
        week = whoop[(whoop['date'] > rd - timedelta(days=7)) & (whoop['date'] <= rd)]
        if len(week) > 0:
            for col in ['recovery_score', 'hrv_rmssd', 'resting_hr']:
...
def get_lab_context():
    """Pega último exame relevante do Supabase"""
    lab = get_lab_results(long=True)
    if lab.empty:
        return {}
...
def find_similar_rides(ride):
    """Encontra treinos semelhantes no histórico (Supabase)"""
    sessions = get_cycling()
    if sessions.empty:
        return []
    ...
    elif 'moving_time_s' in similar.columns:
        similar['duration_min'] = similar['moving_time_s'] / 60
        dur_col = 'duration_min'
    else:
        return []
...
def generate_suggestions(ride, state, impact, lab_context):
    ...
    if tss > 200 or new_tsb < -30:
        ...
        suggestions.append({
            ...
            'reason': 'Recovery ativo ajuda sem acumular fadiga',
        })
    else:
        suggestions.append({
            'priority': 'low',
            'action': 'Treino normal amanhã',
            'reason': 'Carga leve, sem necessidade de recovery extra',
        })
    ...
    rec_7d = state.get('recovery_score_7d')
...
def print_summary(ride, state, impact, similar, suggestions, lab_context):
    ...
    print(f"  💪 {ride.get('wkg', 0):.2f} W/kg avg | {ride.get('np_wkg', 0):.2f} W/kg NP (peso: {ride.get('weight_used', 68):.1f}kg)")
    if ride.get('ef', 0) > 0:
    ...
    if state.get('recovery_score'):
        print(f"    Recovery: {state['recovery_score']:.0f}%")
    if state.get('hrv_rmssd'):
        print(f"    HRV: {state['hrv_rmssd']:.0f}ms | RHR: {state.get('resting_hr', 0):.0f}bpm")
...
def main():
    parser = argparse.ArgumentParser(description='Análise pós-treino (Supabase)')
    ...
    print("\nCarregando contexto do Supabase...")
    state = get_current_state(ride['date'])
    lab_context = get_lab_context()
    similar = find_similar_rides(ride)

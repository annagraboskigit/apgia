...
try:
    ...
    preset = st.sidebar.radio(
        "Período rápido",
        ["Tudo", "Último ano", "Últimos 6 meses", "Últimos 3 meses", "Custom"],
        horizontal=True,
        index=0,
    )
    if preset == "Tudo":
...
# ═══════════════════════════════════════════════════════════════
    ...
    st.caption(f"📅 {start_date} → {end_date} • {len(wp)} treinos")
...
# ═══════════════════════════════════════════════════════════════
    ...
    st.info("ℹ️ Dados de gradiente são agregados do período completo de records (fev/2024–fev/2026). Filtro de data aplica apenas na distribuição de elevação abaixo.")
    ...
    # Climbing segments if available
    segments_path = DATA_DIR / "climbing_segments.csv"
    if segments_path.exists():
        st.markdown("---")
        st.subheader("Segmentos de Subida")
        segs = pd.read_csv(segments_path)
        if "timestamp" in segs.columns:
            segs["timestamp"] = pd.to_datetime(segs["timestamp"])
            segs = filter_by_date(segs)
        st.caption(f"{len(segs)} segmentos de subida >3 min")

        # Best VAM efforts
        if "vam" in segs.columns:
            top_vam = segs.nlargest(10, "vam")
            display_cols = [c for c in ["timestamp", "vam", "avg_power", "duration_min",
                                         "avg_gradient", "elevation_gain"] if c in top_vam.columns]
            if display_cols:
                st.dataframe(top_vam[display_cols].round(1), use_container_width=True)
...
# ═══════════════════════════════════════════════════════════════
    ...
    st.caption(f"📅 {start_date} → {end_date} • {len(pac)} treinos")
